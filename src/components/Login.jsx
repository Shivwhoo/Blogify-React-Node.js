...
function Login() {
  ...
  const login = async (data) => {
    ...
    try {
      ...
      // OPTIMIZATION: Extract user directly from the login response!
      // Depending on Axios, it might be session.data.data.user or session.data.user
      const user = session?.data?.data?.user || session?.data?.user;
      ...
      // FIX: Dig into the Axios error object to get your custom backend message
      const backendErrorMessage = error.response?.data?.message;
      setError(backendErrorMessage || "Invalid email or password");
    } finally {
      setLoading(false);
    }
  };
